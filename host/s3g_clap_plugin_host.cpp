...
bool guiRequestResize(const clap_host_t* host, uint32_t width,
    uint32_t height)
{
    auto* self = static_cast<Plugin*>(host
...
void EntrySession::deinitialize()
{
    factory_ = nullptr;
...
Plugin::Plugin()
{
    ...
    host_.vendor = "s3g";
    host_.url = "https://github.com/s3g/s3g-clap-max";
    host_.version = "1";
...
bool Plugin::create(const clap_plugin_entry_t* entry,
...
{
    destroy();
    if (!entry || !factory || !factory->create_plugin || !pluginId
        || !hostName) return false;
    entry_ = entry;
    hostName_ = hostName;
    host_.name = hostName_.c_str();
    plugin_ = factory->create_plugin(factory, &host_, pluginId);
    if (!plugin_ || !plugin_->init || !plugin_->init(plugin_)) {
        destroy();
        return false;
    }
...
bool Plugin::create(
    const EntrySession& entrySession,
    const char* pluginId, const char* hostName)
{
    destroy();
...
clap_process_status Plugin::process(
...
bool Plugin::saveState(std::vector<uint8_t>& destination) const
{
...
void Plugin::notifyGuiShowRequested()
{
    guiShowRequested_.store(true, std::memory_order_release);
}

void Plugin::notifyGuiHideRequested()
{
    guiHideRequested_.store(true, std::memory_order_release);
}

void Plugin::notifyGuiClosed(bool wasDestroyed)
{
    guiClosedState_.store(wasDestroyed ? 2u : 1u,
        std::memory_order_release);
}

Plugin* Plugin::fromHost(const clap_host_t* host)
{
    return static_cast<Plugin*>(host ? host->host_data : nullptr);
}

const void* Plugin::hostGetExtension(const clap_host_t*,
...
void Plugin::hostRequestRestart(const clap_host_t* host)
{
    if (auto* self = fromHost(host))
        self->restartRequested_.store(true, std::memory_order_release);
}

void Plugin::hostRequestProcess(const clap_host_t* host)
{
    if (auto* self = fromHost(host))
        self->processRequested_.store(true, std::memory_order_release);
}

void Plugin::hostRequestCallback(const clap_host_t* host)
{
    if (auto* self = fromHost(host))
